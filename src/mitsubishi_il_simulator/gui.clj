...
(defn control-panel []
  ...
  {:fx/type :v-box
   :spacing 10
   :children [{:fx/type :label :text "Program Control" :style "-fx-font-weight: bold;"}
              {:fx/type :h-box
               :spacing 10
               :children [{:fx/type :button :text "Load" :on-action (fn [_] (load-program-from-file))}
              ...
              {:fx/type :h-box
               :spacing 10
               :children [{:fx/type :button :text "Clear Log" :on-action (fn [_] (on-clear-log))}
...
(defn input-panel [state]
  ...
  {:fx/type :v-box
   :spacing 10
   :children [{:fx/type :label :text "Inputs (X)" :style "-fx-font-weight: bold;"}
              {:fx/type :grid-pane
               :hgap 5
               :vgap 5
...
(defn output-panel [state]
  ...
  {:fx/type :v-box
   :spacing 10
   :children [{:fx/type :label :text "Outputs (Y)" :style "-fx-font-weight: bold;"}
              {:fx/type :grid-pane
               :hgap 5
               :vgap 5
               :children (for [i (range 8)]
...
(defn program-editor [state]
  ...
  {:fx/type :v-box
   :spacing 10
   :children [{:fx/type :label :text "IL Program Editor" :style "-fx-font-weight: bold;"}
              {:fx/type :text-area
               ...
               :min-width 900         ; Set minimum width
               :pref-width 900       ; Set preferred width
               :style "-fx-font-family: monospace; -fx-text-fill: black; -fx-control-inner-background: white;-fx-font-weight: bold;"}]})

(defn output-log [state]
  "Create the output log"
  {:fx/type :v-box
   :spacing 10
   :children [{:fx/type :label :text "Output Log" :style "-fx-font-weight: bold;"}
              {:fx/type :text-area
  ...
  {:fx/type :stage
   ...
   :scene {:fx/type :scene
           :root {:fx/type :v-box
                  :spacing 10
                  :padding 10
                  :children [{:fx/type :label :text "Mitsubishi IL Simulator" :style "-fx-font-size: 18px; -fx-font-weight: bold;"}
                             (control-panel)
                             {:fx/type :h-box
                              :spacing 10
